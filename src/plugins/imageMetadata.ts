...
async function addMetadata(node: ImageNode): Promise<void> {
    ...
    const res: any = await sizeOf(
        Buffer.from(await fs.readFile(path.join(process.cwd(), 'public', node.properties.src)))
    );

    if (!res) throw Error(`Invalid image with src "${node.properties.src}"`);
    const { base64 } = await getPlaiceholder(path.join(process.cwd(), 'public', node.properties.src) as unknown as GetPlaiceholderSrc, { size: 10 }); // 10 is to increase detail (default is 4)
